...
async def music_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("❌ <b>الاستخدام:</b>\n<code>/music اسم الأغنية</code>", parse_mode="HTML")
        return
    query = " ".join(context.args)
    await update.message.reply_text(f"🎵 <b>البحث عن:</b> <i>{query}</i>\n\nأرسل رابط مباشر ينتهي بـ .mp3 أو .m4a لأشغله لك.", parse_mode="HTML")
...
async def settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"⚙️ <b>إعدادات:</b>\n\n🤖 الرد الآلي: {auto_status}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
